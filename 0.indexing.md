# ##### INDEXING # #####
Nama Tabel : produk
Kolom Index (Dexie Format) : "++id, &kode_produk, nama_produk, id_kategori, updated_at" || Gunakan simbol & pada kode_produk (Unique Index). Ini memastikan tidak ada dua barang dengan barcode yang sama.
Alasan & Fungsi : Scan barcode super cepat dan validasi input barang baru.

Nama Tabel : satuan_produk
Kolom Index (Dexie Format) : "++id, id_produk, is_deleted"
...
Kolom Index (Dexie Format) : "++id, nomor_struk, dibuat_pada, id_member, status_sinkronisasi, [status_sinkronisasi+dibuat_pada]" || WAJIB tambah status_sinkronisasi (0 atau 1). || Karena aplikasimu offline-first, Dexie perlu mencari dengan cepat mana transaksi yang belum terkirim ke Firebase Firestore. Tanpa index ini, proses sinkronisasi akan berat.
...
Kolom Index (Dexie Format) : "++id, dibuat_pada, aksi, id_pengguna, tabel_terkait" || Tambahkan id_pengguna || Penting untuk audit. Kamu bisa memfilter "Apa saja yang dilakukan oleh Kasir A hari ini?".
